...
\begin{answer}
  \begin{table}[H]
    ...
    Notably, forward-backward (and with optimizer) steps take a lot longer than just forward steps, which makes
    sense because of the gradient computation overhead and other tracking measurements that forward doesn't have to deal with.
    The variability across runs is extremely small and the run times are very consistent, as seen by the small standard deviations.
\end{answer}
