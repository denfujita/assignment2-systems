...
MEM_PROFILE = bool(int(os.environ.get("CS336_MEM_PROFILE", "0")))
...
def mem_range(name: str):
    if not MEM_PROFILE:
        yield
        return

    before = _mem_mib()
    peak_before = _peak_mem_mib()
    yield
    after = _mem_mib()
    peak_after = _peak_mem_mib()

    delta = after - before
    peak_delta = peak_after - peak_before

    MEM_LOG.append((name, delta, peak_delta))

    print(
        f"[MEM] {name}: "
        f"delta={delta:.2f} MiB, "
        f"peak_delta={peak_delta:.2f} MiB"
    )
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        Returns:
            FloatTensor of shape `(batch_size, sequence_length, d_model)`.
        """
        with mem_range("block.ln1"):
            ln1_out = self.ln1(x)

        with mem_range("block.attn"):
            x_attn = self.attn(ln1_out)

        with mem_range("block.residual_attn"):
            attn_sublayer_output = x + x_attn

        with mem_range("block.ln2"):
            ln2_out = self.ln2(attn_sublayer_output)

        with mem_range("block.ffn"):
            x_ffn = self.ffn(ln2_out)

        with mem_range("block.residual_ffn"):
...
class SwiGLU(nn.Module):
    ...
    def forward(self, x):
        with mem_range("ffn.w1"):
            w1_out = self.w1(x)

        with mem_range("ffn.silu"):
            silu_out = silu(w1_out)

        with mem_range("ffn.w3"):
            w3_out = self.w3(x)

        with mem_range("ffn.gate_mul"):
            gated = silu_out * w3_out

        with mem_range("ffn.w2"):
            out = self.w2(gated)
...
def scaled_dot_product_attention(
    ...
    with mem_range("sdpa.attention_scores"):
        attention_scores = einsum(Q, K, "... query d_k, ... key d_k -> ... query key") / math.sqrt(d_k)

    if mask is not None:
        with mem_range("sdpa.mask"):
            attention_scores = torch.where(mask, attention_scores, float("-inf"))

    with mem_range("sdpa.softmax"):
        attention_weights = softmax(attention_scores, dim=-1)

    with mem_range("sdpa.weighted_values"):
...
class CausalMultiHeadSelfAttention(nn.Module):
    ...
    def forward(
        ...
        *batch_dims, sequence_length, d_model = x.size()
        ...
        with mem_range("attn.q_proj"):
            Q = self.q_proj(x)

        with mem_range("attn.k_proj"):
            K = self.k_proj(x)

        with mem_range("attn.v_proj"):
            V = self.v_proj(x)

        with mem_range("attn.rearrange_qkv"):
            Q, K, V = (
        ...
        if self.positional_encoder is not None:
            ...
            with mem_range("attn.rope_q"):
                Q = self.positional_encoder(Q, token_positions)

            with mem_range("attn.rope_k"):
                K = self.positional_encoder(K, token_positions)

        with mem_range("attn.causal_mask"):
        ...
        with mem_range("attn.scaled_dot_product_attention"):
            attn_output = scaled_dot_product_attention(K=K, Q=Q, V=V, mask=causal_mask)

        with mem_range("attn.rearrange_output"):
            attn_output = rearrange(attn_output, "batch heads seq d_v -> batch seq (heads d_v)").contiguous()

        with mem_range("attn.output_proj"):
